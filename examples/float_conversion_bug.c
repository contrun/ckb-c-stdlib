...
__attribute__((noinline)) double convert_integer_to_number(long long i) {
  printf("Integer before conversion: %lld\n", i);
  double n = (double)(i);
  printf("Number after conversion: %.6f\n", n);
  return n;
}

int main() {
  long long i = -9223372036854775808uLL;
  convert_integer_to_number(i);
  return 0;
}
